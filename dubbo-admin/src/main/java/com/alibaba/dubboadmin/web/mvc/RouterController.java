...
import com.alibaba.dubboadmin.web.container.SpringUtil;
...
public class RouterController {
    ...
    public String addresswithIDRouter(@RequestParam Map<String, String> params,
    ...
    public String addressWithIDandAction(@PathVariable("ip") String ip,
                                         @PathVariable("type") String type,
    ...
    // 针对服务级别的，某个服务/类型（如提供者）/{数字/行为，如add}
    ...
    public String appRouter(@PathVariable("app") String app,
        ...
        if (name != null) {
            // 从Spring 中取出Controller
            Object controller = SpringUtil.getBean(name);
            if (controller != null) {
                try {
                    // 反射调用
                    Method index = controller.getClass().getDeclaredMethod("index", HttpServletRequest.class, HttpServletResponse.class, Model.class);
    ...
    public String appAction(@RequestParam Map<String, String> params,
        ...
        // 从缓存中获取类型对应的Contoller
        String name = WebConstants.mapper.get(type);
        if (name != null) {
            // Spring中获取对应的Contoller
            Object controller = SpringUtil.getBean(name);
            if (controller != null) {
                // POST 请求
                if (request.getMethod().equals("POST")) {
                    Method[] methods = controller.getClass().getDeclaredMethods();
                    // 遍历Controller中的方法，看是否有匹配 action 指定的方法
                    ...
                    try {
                        ...
                        if (StringUtils.isNumeric(action)) {
                            ...
                            // 反射调用
                            Method m = controller.getClass().getDeclaredMethod(action, HttpServletRequest.class, HttpServletResponse.class, Model.class);
    ...
    public String appActionWithIdandAction(@PathVariable("app") String app,
                                           @PathVariable("service") String service,
                                           @PathVariable("type") String type,
                                           @PathVariable("id") String id,
                                           @PathVariable("action") String action,
            ...
            if (controller != null) {
                try {
                    ...
                    if (StringUtils.isNumeric(id)) {
                        ...
                        // 反射调用
                        Method m = controller.getClass().getDeclaredMethod(action, Long[].class, HttpServletRequest.class, HttpServletResponse.class, Model.class);
