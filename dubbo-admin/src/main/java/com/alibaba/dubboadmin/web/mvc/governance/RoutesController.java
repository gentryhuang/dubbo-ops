...
public class RoutesController extends BaseController {
    ...
    public String show(@PathVariable("id") Long id,
        ...
        try {
            ...
            model.addAttribute("methods", CollectionUtils.sort(new ArrayList<>(providerService.findMethodsByService(route.getService()))));
    ...
    public String add(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (service != null && service.length() > 0 && !service.contains("*")) {
            ...
            // 获取服务对应的方法列表
            model.addAttribute("methods", CollectionUtils.sort(new ArrayList<>(providerService.findMethodsByService(service))));
        } else {
    ...
    public String update(@PathVariable("id") Long id,
        ...
        if (idStr != null && idStr.length() > 0) {
            ...
            if (StringUtils.isNotEmpty((String) request.getParameter("name"))) {
                ...
                for (String[] names : when_names) {
                    when_name2valueList.put(names[0], request.getParameter(names[1]));
                    notWhen_name2valueList.put(names[0], request.getParameter(names[2]));
                }
